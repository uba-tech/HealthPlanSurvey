<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Skin_Mobile.ascx.cs" Inherits="DNNGo.Modules.xPlugin.Skin_Mobile" %>
     ...
     jQuery(document).ready(function ($) {
         ...
         $('#gomenu<%=ClientID %>').mobile_menu({
		  ...
		  $('#gomenu<%=ClientID %>').find("a[href='javascript:;']").on("click",function () {
			  $('#gomenu<%=ClientID %>').find(".mm-close.mm-btn").click();
		  })
     });
</script>